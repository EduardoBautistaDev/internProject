    ...
    override suspend fun getHeaderInfo(): Result<HeaderInfo, Throwable> =
        try {
            with(infoApi.getInfo()) {
                when {
                    //DONE("Convert DTO into domain model")
                    header != null -> Ok(headerInfoMapper.toDomain(header, items))
                    //DONE("Convert to error")
                    error != null -> Err(IllegalStateException("API error: $error"))
                    //DONE("Convert to error")
                    else -> Err(IllegalStateException("Unexpected response structure"))
                }
            }
